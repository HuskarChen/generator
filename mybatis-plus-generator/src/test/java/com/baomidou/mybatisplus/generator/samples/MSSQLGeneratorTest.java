...
//        .schema(scanner("模式名"))
    ...
    private static StrategyConfig strategyConfig(String moduleName) {
        StrategyConfig sc = new StrategyConfig
            ...
            .build();
        sc.entityBuilder().columnNaming(NamingStrategy.underline_to_camel).enableLombok().enableTableFieldAnnotation().fileOverride();
        sc.controllerBuilder().enableRestStyle().enableHyphenStyle().fileOverride();
        sc.serviceBuilder().fileOverride();
        sc.mapperBuilder().fileOverride();
        return sc;
    }
